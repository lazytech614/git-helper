...
function makeEmptyTable(cols: number, rows: number): TableState {
  return {
...
// Markdown generation
// ---------------------------------------------------------------------------
function generateMarkdown(table: TableState, padding: boolean, pretty: boolean): string {
  ...
  if (pretty) {
    ...
    const headerRow = "| " + headers.map((h, i) => cell(h, colWidths[i], alignments[i])).join(" | ") + " |"
...
// Export helpers
// ---------------------------------------------------------------------------
function exportAs(table: TableState, format: ExportFormat, padding: boolean, pretty: boolean): string {
...
// ---------------------------------------------------------------------------
function parseMarkdown(md: string): TableState | null {
...
// ---------------------------------------------------------------------------
function renderTableHTML(table: TableState): string {
  const { headers, alignments, rows } = table
  const th = headers
  ...
  const trs = rows
    .map(
      (r) =>
        "<tr>" +
        r.map((v, i) => `<td style="text-align:${alignments[i]}">${escape(v ?? "")}</td>`).join("") +
        "</tr>"
    )
    .join("")
