...
export function detectBase(raw: string): { base: Base; stripped: string; detected: string } | null {
  ...
  if (/^0x/i.test(s)) return { base: 16, stripped: s.slice(2), detected: "Hexadecimal (0x prefix)" }
  ...
  if (/^[01]+$/.test(s) && s.length >= 4)
    return { base: 2, stripped: s, detected: "Binary" }
  ...
  // Pure decimal
  if (/^\d+$/.test(s))
    return { base: 10, stripped: s, detected: "Decimal" }
...
export function validateInput(value: string, base: Base): string | null {
  ...
  if (!BASE_PATTERNS[base].test(value.trim())) {
    ...
    const invalid = value.trim().split("").find((c) => !BASE_PATTERNS[base].test(c))
...
export function convert(
  ...
  if (validationError) {
    return { binary: "", octal: "", decimal: "", hex: "", decimalValue: null, error: validationError }
  }
  ...
  try {
    ...
    return { binary: "", octal: "", decimal: "", hex: "", decimalValue: null, error: "Conversion failed" }
  }
}
...
export function charToInfo(char: string): { ascii: string; binary: string; hex: string; decimal: string } | null {
...
export function bitwiseOp(
  ...
  switch (op) {
    ...
    case "XOR":    return a ^ b
    case "NOT":    return ~a
    case "LSHIFT": return a << BigInt(shiftAmount)
    case "RSHIFT": return a >> BigInt(shiftAmount)
  }
}
...
export function arithmetic(
  ...
  try {
    switch (op) {
      ...
      case "-": return a - b
      case "*": return a * b
      case "/": return b === BigInt(0) ? null : a / b
    }
  } catch {
    return null
  }
